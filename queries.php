...
{                         
  ...
  if($con)
  {
    ...
    $venue_data=array('city'=>$city,'eventtype'=>$eventtype,'venuetype'=>$venuetype,'venuename'=>$venuename,'locality'=>$locality,'capacity'=>$capacity,'cost'=>$cost,'image'=>$image);
                            
    $collection->save($venue_data);
		...
		<div class="row">
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12" id="ver-nav">
				...
				<ul class="nav nav-pills nav-stacked">
				  ...
				  <li role="presentation" class="active"><a href="queries.php">Contact-Us Details</a></li>
 ...
 <?php
    ...
    $conn = new MongoClient();            //Connect To MongoDB
        if($conn){
            ...
            $collection=$database->contactlist;       //Connect to question_bank collection

            $result=$collection->find();

foreach ($result as $query) {

echo "<table class='table table-hover'><tr><th>Name</th><th>Email Id</th><th>Subject</th><th>Message</th></tr>";
echo '<tr><th>'.$query["Name"].'</th><th>'.$query["Email"].'</th><th>'.$query["Subject"].'</th><th>'.$query["Message"].'</th></tr>';
echo'<br>';
}
echo"</table>";
}
